...
const STATUS_CONF: Record<string, { txt: string; color: string; bg: string }> = {
  ouvert:  { txt: 'Disponible',  color: '#1a2e1e',              bg: '#f6b74d' },
  ...
  return (
    <Link href={`/destinations/${v.slug}`} style={{ textDecoration: 'none', display: 'block' }}>
      <motion.article
        ...
        style={{ position: 'relative', height: '80vh', overflow: 'hidden', marginBottom: '3px', background: '#1a3320', cursor: 'pointer' }}
      >
        <Image
...
export default function DestinationsPage() {
  ...
  return (
    <div style={{ background: '#1a2e1e', minHeight: '100dvh' }}>
      <PageHero
        image="/images/15.jpg"
      ...
      <div style={{ padding: '80px 56px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '32px', borderTop: '1px solid rgba(255,255,255,0.07)' }}>
        ...
        <Link href="/candidater" style={{ fontFamily: J, fontSize: '10px', letterSpacing: '0.22em', textTransform: 'uppercase', background: '#f6b74d', color: '#1a2e1e', padding: '16px 44px', textDecoration: 'none', fontWeight: 500 }}>
          Candidater →
        </Link>
      </div>
    </div>

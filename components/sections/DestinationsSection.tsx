...
export default function DestinationsSection() {
  ...
  return (
    <section style={{ background: '#1a2e1e' }} className="section-pad">
            ...
            <motion.div key={voyage.id} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8, delay: i * 0.12, ease: [0.22, 1, 0.36, 1] }}
              ...
              <Link href={`/destinations/${voyage.slug}`} style={{ textDecoration: 'none', display: 'block' }}>
                ...
                <div style={{ position: 'relative', aspectRatio: '3/4', overflow: 'hidden', marginBottom: '20px' }}>
                  ...
                  {/* Status */}
                  <div style={{ position: 'absolute', top: '20px', left: '20px', fontFamily: M, fontSize: '9px', textTransform: 'uppercase', letterSpacing: '0.18em', background: voyage.status === 'ouvert' ? '#f6b74d' : 'rgba(0,0,0,0.45)', color: voyage.status === 'ouvert' ? '#1a2e1e' : 'rgba(255,255,255,0.6)', padding: '5px 12px', fontWeight: 500 }}>
